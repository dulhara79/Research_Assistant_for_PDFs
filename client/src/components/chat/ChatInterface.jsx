...
export default function ChatInterface({ activeDoc, messages, loading, onSend, question, setQuestion }) {
    ...
    return (
        <div className="flex-1 flex flex-col h-full bg-slate-950/50">
            ...
            <div className="h-16 border-b border-slate-800 flex items-center px-6 bg-slate-900/80 backdrop-blur-md">
                ...
                <h2 className="font-semibold text-slate-200 truncate">{activeDoc.title}</h2>
            </div>

            {/* Messages Area */}
            <div className="flex-1 overflow-y-auto p-4 space-y-6 scroll-smooth" ref={scrollRef}>
                {messages.map((msg, idx) => (
                    <div key={idx} className={`flex gap-4 animate-slideUp ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                            msg.role === 'user' ? 'bg-indigo-600' : 'bg-emerald-600'
                        }`}>
                            {msg.role === 'user' ? <MessageSquare className="w-4 h-4 text-white" /> : <History className="w-4 h-4 text-white" />}
                        </div>
